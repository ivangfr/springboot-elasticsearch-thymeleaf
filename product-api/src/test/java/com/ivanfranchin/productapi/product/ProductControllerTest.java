...
class ProductControllerTest {
    ...
    void getProductWhenNotFoundShouldReturn404() throws Exception {
        when(productService.validateAndGetProductById("123")).thenThrow(new ProductNotFoundException("123"));
    ...
    void createProductShouldReturn201() throws Exception {
        ...
        CreateProductRequest request = new CreateProductRequest("New", "Desc", BigDecimal.TEN, Set.of("cat1"));
    ...
    void searchProductsShouldReturnPage() throws Exception {
        ...
        mockMvc.perform(put("/api/products/search")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isArray());
