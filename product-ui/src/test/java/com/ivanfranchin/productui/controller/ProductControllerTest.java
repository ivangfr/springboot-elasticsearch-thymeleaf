...
class ProductControllerTest {
  ...
  void getProductsShouldReturnProductsView() throws Exception {
    ...
    when(productApiClient.listProductsByPage(isNull(), isNull(), eq("created,desc")))
        .thenReturn(page);
  ...
  void searchProductsWithTextShouldCallSearch() throws Exception {
    MyPage<Product> page = new MyPage<>(List.of(), 0, 0, 10, 0, true, true);
    when(productApiClient.searchProductsByPage(any(), isNull(), isNull(), eq("created,desc")))
        .thenReturn(page);

    mockMvc
        .perform(get("/").param("text", "laptop"))
        .andExpect(status().isOk())
        .andExpect(view().name("products"))
        .andExpect(model().attributeExists("products"));

    verify(productApiClient)
        .searchProductsByPage(any(SearchDto.class), isNull(), isNull(), eq("created,desc"));
  }
  ...
  void createProductWithValidationErrorShouldReturnForm() throws Exception {
    mockMvc
        .perform(
            post("/products").param("name", "").param("description", "Desc").param("price", "10"))
        .andExpect(status().isOk())
        .andExpect(view().name("productCreate"));
  }
  ...
  void deleteProductShouldRedirect() throws Exception {
    mockMvc
        .perform(post("/products/123/delete"))
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectedUrl("/"));
