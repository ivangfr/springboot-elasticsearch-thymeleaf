...
echo "----------------"
curl -X PUT --fail localhost:9200/"${new_index_name}" -H "Content-Type: application/json" -d @"${new_mapping_file}"

echo
echo "--------"
echo "Re-index"
echo "--------"
curl -X POST --fail localhost:9200/_reindex -H 'Content-Type: application/json' \
...
echo "----------------"
curl -X DELETE --fail localhost:9200/"${old_index_name}"